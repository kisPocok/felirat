...
describe("Movie/Helper", function() {
    ...
    describe("removeFileExtension", function() {
        ...
        it("withoutExtensionShouldPassOriginalValue", function() {
            ...
            expect(actual).to.be.equal(data);
        });
    });
    ...
    describe("srtFileName", function () {
        it("shouldBeValidFileName", function() {
            ...
            expect(actual).to.be.equal(fileName + '.' + lang + '.srt');
        });
    });

    describe("variables", function () {
        it("validSubtitleExtension", function() {
            expect(Helper.validSubtitleExtension).to.be.equal('srt');
        });
        it("validVideoExtensions", function() {
            expect(Helper.validVideoExtensions.length).to.be.equal(2);
            expect(Helper.validVideoExtensions[0]).to.be.equal('mp4');
            expect(Helper.validVideoExtensions[1]).to.be.equal('mkv');
        });
    });
